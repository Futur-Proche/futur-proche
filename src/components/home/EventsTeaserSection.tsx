...
const EventCard = ({ ev, past = false }: { ev: Ev; past?: boolean }) => {
      ...
      {ev.image_url ? (
        <div className="relative h-40 overflow-hidden">
          <img
            src={ev.image_url}
            alt={ev.titre}
            className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${past ? "grayscale-[40%] opacity-90" : ""}`}
            loading="lazy"
          />
          ...
          {past && (
            <span className="absolute top-3 right-3 text-[9px] font-mono uppercase tracking-wider bg-white/90 px-2 py-1 rounded-full" style={{ color: "hsl(228 56% 10%)" }}>
              Passé
            </span>
          )}
        </div>
    ...
    <section className="section-cream">
      <div className="container mx-auto px-6 lg:px-12 py-20 md:py-24">
        ...
        <div className="mb-12">
          ...
          {upcomingEmpty ? (
            <div
              className="rounded-2xl bg-white p-8 md:p-10 text-center max-w-2xl mx-auto"
              style={{ border: "1px solid hsl(228 10% 85%)" }}
            >
              <span className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "hsl(186 60% 32%)" }}>
                — Bientôt
              </span>
              <h4 className="mt-3 text-xl md:text-2xl font-grotesk font-semibold" style={{ color: "hsl(228 56% 10%)" }}>
